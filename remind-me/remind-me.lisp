...
(in-package #:remind-me)

;;; "remind-me" goes here. Hacks and glory await!



(defvar *site* (make-instance 'acceptor :port 8000))

;;; This will be removed since I am going to write static css for this app.
...
(define-easy-handler (create-user :uri "/create-user") ()
  (standard-page (:title "Create a new assistant")
    ...
    (:div :class "span-8" (:form :class "register" :id "new-user" :action "/new-user" :method :post :enctype "multipart/form-data"
                                 (str (input-for "name" "string"))
                                 (str (input-for "password" "password"))
...
;;; Lets see if we can write a macro for form input and see what we can refactor
;   (input-for "name" "string") should expand to
;   (:p "name" :br (:input :type "string" :name "name" :label "name"))

(defmacro input-for (name type)
  (with-gensyms (nam typ)
    (let ((nam name)
           (typ type))
       `(with-html-output-to-string (s) (:p ,nam :br (:input :type ,typ :name ,nam :label ,nam))))))
